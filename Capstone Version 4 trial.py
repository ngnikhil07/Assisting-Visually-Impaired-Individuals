...
while cap.isOpened():
    ...
    # Reading frames
    framed = vs.read()
    framed = imutils.resize(framed, width=500)
    height, width, channels = framed.shape

    # Converting frames to blobs using mean standardization
    blob = cv2.dnn.blobFromImage(image=framed,
    ...
    for detection in network_output[0, 0]:
        ...
        if score > SCORE_THRESHOLD:
            ...
            cv2.rectangle(img=framed,
                          rec=(left, top, right, bottom),
                          color=COLORS[class_index],
                          thickness=4,
                          lineType=cv2.LINE_AA)

            cv2.putText(img=framed,
                        text=label,
