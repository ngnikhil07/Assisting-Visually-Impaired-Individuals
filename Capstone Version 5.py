...
while cap.isOpened():
    ...
    if len(faces) > 0:
            ...
            # grabcut thread
            grabcut = GrabCut(image, x, y, w, h, 0.3, Queue(1))
            grabcut.start()

            # blur thread
            blur = BlurBkg(Queue(1), image)
            blur.start()
            ...
            frame_with_face = image * mask[:, :, np.newaxis]

            # merge frame blurred and the face
            image = np.where((mask[:, :, np.newaxis] == 2) | (mask[:, :, np.newaxis] == 0), frame_blurred, frame_with_face)
            ...
            blur = BlurBkg(Queue(1), image)
            blur.start()
            image = blur.blur_queue.get()

            # check face detected
            cv2.circle(image, (50, 50), 10, (0, 0, 255), -1)
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            cv2.putText(image, "{}: {:.2f}".format(indication, var), (505, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
